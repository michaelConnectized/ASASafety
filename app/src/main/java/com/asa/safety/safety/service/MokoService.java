package com.asa.safety.safety.service;
...
public class MokoService extends Service implements MokoConnStateCallback, MokoOrderTaskCallback {
    ...
    public OrderTask getFastMode() {
        WriteConfigTask writeConfigTask = new WriteConfigTask(this, OrderTask.RESPONSE_TYPE_WRITE_NO_RESPONSE);
        writeConfigTask.setData(ConfigKeyEnum.GET_FAST_MODE);
        return writeConfigTask;
    }

    public OrderTask setSlowMode(int advInterval, int advTxPower, int txPower) {
        WriteConfigTask writeConfigTask = new WriteConfigTask(this, OrderTask.RESPONSE_TYPE_WRITE_NO_RESPONSE);
        writeConfigTask.setSlowMode(advInterval, advTxPower, txPower);
        return writeConfigTask;
    }

    public OrderTask getSlowMode() {
        WriteConfigTask writeConfigTask = new WriteConfigTask(this, OrderTask.RESPONSE_TYPE_WRITE_NO_RESPONSE);
        writeConfigTask.setData(ConfigKeyEnum.GET_SLOW_MODE);
        return writeConfigTask;
    }

    public OrderTask setFastMode(boolean FastEnable, int timeout, int advInterval, int advTxPower, int txPower) {
        WriteConfigTask writeConfigTask = new WriteConfigTask(this, OrderTask.RESPONSE_TYPE_WRITE_NO_RESPONSE);
        writeConfigTask.setFastMode(FastEnable, timeout, advInterval, advTxPower, txPower);
        return writeConfigTask;
    }
     ...
     * @Description 设置连接状态
     */
    public OrderTask setLEDInfo(boolean controlEnable, boolean always, int timeout, int LedOn, int LedOff) {
        WriteConfigTask writeConfigTask = new WriteConfigTask(this, OrderTask.RESPONSE_TYPE_WRITE_NO_RESPONSE);
        writeConfigTask.setLEDInfo(controlEnable, always, timeout, LedOn, LedOff);
        return writeConfigTask;
    }
